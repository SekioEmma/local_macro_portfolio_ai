...
def main() -> None:
    ...
    eval_case = route_eval_case(eval_case, answer_style, user_question)
    ...
    if result.get("answer"):
        guarded = apply_deterministic_answer_guardrails(
            result["answer"],
            context_pack,
            user_question=user_question,
            answer_style=answer_style,
    ...
    if (
        mode == "local_http"
        and result.get("status") == "ok"
        and is_market_top_pullback_case(eval_case)
...
def _build_answer_prompt(
    ...
    prompt_eval_case = route_eval_case(eval_case, answer_style, user_question)
    if compact_prompt:
